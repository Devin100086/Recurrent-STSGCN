...
def position_embedding(data,
    ...
    if temporal:
        # shape is (1, T, 1, C)
        temporal_emb = torch.empty(1, input_length, 1, embedding_size)
        torch.nn.init.xavier_uniform_(temporal_emb, gain=0.0003)
    if spatial:
        # shape is (1, 1, N, C)
        spatial_emb = torch.empty(1, 1, num_of_vertices, embedding_size)
...
class gcn_operation(nn.Module):
    def __init__(self, num_of_filter, num_of_features, num_of_vertices,
                 activation):
        '''
        GCN操作
    ...
    def forward(self, data, adj):
        ...
        # shape is (3N, B, C)
        data = torch.reshape(data, (data.shape[0], -1, self.num_of_features))
...
class stsgcm(nn.Module):
    def __init__(self, filters, num_of_features, num_of_vertices,
        ...
        self.gcn_operation = nn.ModuleList([gcn_operation(filters[i], feture[i], num_of_vertices, activation)
    ...
    def forward(self, data, adj):
        ...
        # shape is (N, B, C')
        result, _ = torch.max(torch.cat(need_concat, dim=0), dim=0)
...
if __name__ == '__main__':
    ...
    num_of_vertices = 10
    activation = 'relu'
    gcn = gcn_operation(num_of_filter, num_of_features, num_of_vertices, activation)
    ...
    num_of_vertices = 10
    activation = 'GLU'
    stsgcm = stsgcm(filters, num_of_features, num_of_vertices, activation)
